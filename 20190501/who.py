...
class TalkBack:
	def __init__(self, script_path):
		...
		self.quaternion_get_into_the_door = [0,0,0,1]
		self.quaternion_get_into_the_room=[0,0,-0.731947,0.681361]
		self.pos_get_into_the_door = [1,0,0]
		self.pos_get_into_the_room=[1,0,0]
		self.euler_point=[0,0,-1.50]
	...
	def findperson(self):
		...
		while (time.time()-begin_findperson<60.0):
			...
			if(len(self.face_rects)<1  and time.time()-begin_findperson<40.0):
	...
	def talkback(self,msg):
		...
		#print command
		if(self.people_num < 3 and self.talk_flag == 1):
